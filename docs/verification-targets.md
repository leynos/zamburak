...
| IFC propagation                 | Monotonic dependency propagation, no missing opcode rules, and Track A observer compatibility parity with bounded disabled or no-op overhead | property tests, opcode coverage tests, compatibility probes, benchmark probes | coverage report, propagation fixtures, `tests/compatibility/`, `tests/benchmarks/` | block merge on uncovered opcode or failed Track A parity or overhead gate |
