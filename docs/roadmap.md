...
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| 1.1.1 | `policies/schema.json`, `crates/zamburak-policy/src/policy_def.rs`, `crates/zamburak-policy/src/engine.rs`                                | Canonical schema v1 is enforced fail-closed.                           |
| 1.1.2 | `crates/zamburak-policy/src/migration.rs`, `tests/compatibility/`, `tests/security/`                                                      | Migration transforms are explicit, test-covered, and auditable.        |
| 1.1.3 | `crates/zamburak-core/src/authority.rs`, `crates/zamburak-policy/src/engine.rs`, `tests/security/`                                        | Authority lifecycle transitions are enforced and validated.            |
| 1.2.1 | `rust-toolchain.toml`, `Makefile`, `docs/tech-baseline.md`                                                                                | Baseline config and baseline documentation remain synchronized.        |
| 1.2.2 | `.github/workflows/`, `Makefile`, `docs/verification-targets.md`                                                                          | Phase-gate suites are wired as merge-blocking checks.                  |
| 1.2.3 | `scripts/`, `scripts/tests/`, `docs/scripting-standards.md`                                                                               | Script delivery follows defined runtime and testing standards.         |
| 1.3.1 | `.github/workflows/`, `tests/compatibility/`, `tests/integration/`, `tests/security/`                                                     | Phase 2 start is blocked until design-contract suites pass.            |
| 1.4.1 | `.gitmodules`, `third_party/full-monty/`, `docs/monty-fork-policy.md`                                                                     | `full-monty` submodule exists with explicit fork-policy constraints.   |
| 1.4.2 | `Makefile`, `scripts/`, `docs/adr-001-monty-ifc-vm-hooks.md`                                                                              | `make monty-sync` provides repeatable fork sync plus verification.     |
| 1.5.1 | `third_party/full-monty/`, `tests/compatibility/`                                                                                         | Stable runtime IDs survive suspend or resume and dump or load cycles.  |
| 1.5.2 | `third_party/full-monty/`, `tests/compatibility/`, `tests/security/`                                                                      | Generic observer events exist with no-op semantic parity checks.       |
| 1.5.3 | `third_party/full-monty/`, `tests/compatibility/`                                                                                         | Snapshot extension seam round-trips opaque embedder state safely.      |
| 1.5.4 | `tests/compatibility/`, `tests/benchmarks/`, `docs/verification-targets.md`                                                               | Track A compatibility and overhead invariants are enforced in gates.   |
| 1.6.1 | `crates/zamburak-monty/src/`, `src/`, `tests/integration/`                                                                                | Governed execution path wraps `full-monty` through one adapter API.    |
| 1.6.2 | `crates/zamburak-core/src/`, `tests/security/`                                                                                            | IFC dependency graph and propagation rules are implemented and tested. |
| 1.6.3 | `crates/zamburak-monty/src/observer.rs`, `crates/zamburak-core/src/`, `tests/integration/`                                                | Observer events drive complete IFC updates in normal and strict modes. |
| 1.6.4 | `crates/zamburak-monty/src/external_call.rs`, `crates/zamburak-policy/src/engine.rs`, `tests/security/`                                   | Every external call is policy-gated before effect execution.           |
| 1.6.5 | `tests/compatibility/`, `tests/security/`, `tests/integration/`                                                                           | Compatibility, security, and snapshot-governance suites are active.    |
| 1.7.1 | `docs/monty-fork-policy.md`, `docs/zamburak-engineering-standards.md`                                                                     | Track A patch-budget and naming constraints are enforced in review.    |
| 1.7.2 | `third_party/full-monty/`, `docs/adr-001-monty-ifc-vm-hooks.md`                                                                           | Track A commits are upstream-shaped and mapped to upstream PRs.        |
| 1.7.3 | `Makefile`, `scripts/`, `docs/monty-fork-policy.md`                                                                                       | Range-diff drift checks are repeatable and escalation-triggered.       |
| 2.1.1 | `crates/zamburak-core/src/trust.rs`, `crates/zamburak-core/src/capability.rs`, `crates/zamburak-policy/src/engine.rs`                     | Integrity, confidentiality, and authority remain separate types.       |
| 2.1.2 | `crates/zamburak-sanitizers/src/`, `crates/zamburak-policy/src/engine.rs`, `tests/security/`                                              | Verification and endorsement semantics are deterministic and enforced. |
| 2.2.1 | `crates/zamburak-core/src/control_context.rs`, `crates/zamburak-interpreter/src/external_call.rs`, `crates/zamburak-policy/src/engine.rs` | Effect checks include execution-context summaries.                     |
| 2.2.2 | `tests/security/`, `tests/integration/`                                                                                                   | Control-context side-channel regressions are covered.                  |
| 3.1.1 | `crates/zamburak-core/src/dependency_graph.rs`, `crates/zamburak-core/src/container_state.rs`, `tests/property/`                          | Mutable containers use versioned, alias-safe provenance state.         |
| 3.1.2 | `crates/zamburak-interpreter/src/opcodes.rs`, `tests/property/`, `tests/security/`                                                        | Supported opcode and built-in propagation coverage is complete.        |
| 3.2.1 | `crates/zamburak-core/src/summary.rs`, `crates/zamburak-core/src/witness.rs`, `crates/zamburak-policy/src/engine.rs`                      | Summary fast path and bounded witness behaviour are implemented.       |
| 3.2.2 | `crates/zamburak-policy/src/decision.rs`, `crates/zamburak-policy/src/audit.rs`, `tests/security/`                                        | Overflow handling is fail-closed and explanations are redacted.        |
| 4.1.1 | `crates/zamburak-tools/src/catalogue.rs`, `policies/`, `tests/compatibility/`                                                             | Tool binding enforces pinned schema and documentation hashes.          |
| 4.1.2 | `crates/zamburak-tools/src/mcp_bridge.rs`, `crates/zamburak-policy/src/engine.rs`, `tests/security/`                                      | MCP trust classes enforce per-server capability budgets.               |
| 4.2.1 | `crates/zamburak-tools/src/`, `crates/zamburak-agent/src/confirmation.rs`, `tests/integration/`                                           | Irreversible operations require valid draft-to-commit lineage.         |
| 5.1.1 | `crates/zamburak-agent/src/planner.rs`, `crates/zamburak-tools/src/llm_adapter.rs`, `crates/zamburak-policy/src/engine.rs`                | P-LLM and Q-LLM calls use explicit sink-policy signatures.             |
| 5.1.2 | `crates/zamburak-interpreter/src/external_call.rs`, `crates/zamburak-tools/src/llm_adapter.rs`, `crates/zamburak-policy/src/audit.rs`     | Three-point sink enforcement is active with audit linkage.             |
| 5.2.1 | `tests/integration/`, `tests/security/`                                                                                                   | Privacy-boundary behaviour is validated end to end.                    |
| 5.2.2 | `crates/zamburak-agent/src/planner.rs`, `crates/zamburak-tools/src/llm_adapter.rs`                                                        | Local-only profile is supported through the same policy interfaces.    |
| 6.1.1 | `crates/zamburak-policy/src/audit.rs`, `crates/zamburak-cli/src/commands/audit.rs`, `tests/integration/`                                  | Audit defaults are confidentiality-first and summary-based.            |
| 6.1.2 | `crates/zamburak-policy/src/audit_chain.rs`, `tests/integration/`, `tests/security/`                                                      | Tamper-evident chaining and retention enforcement are active.          |
| 6.2.1 | `.github/workflows/`, `tests/property/`, `tests/security/`                                                                                | Mechanistic and regression suites are merge-blocking gates.            |
| 6.2.2 | `tests/benchmarks/`, `scripts/`, `.github/workflows/`                                                                                     | Model-in-loop adversarial benchmark trends are produced in CI.         |
| 6.3.1 | `scripts/`, `scripts/tests/`                                                                                                              | New scripts comply with Cyclopts, Cuprum, and Pathlib standards.       |
| 6.3.2 | `scripts/tests/`, `.github/workflows/`, `docs/scripting-standards.md`                                                                     | Script tests and CI wiring are deterministic and enforced.             |
| 6.3.3 | `scripts/`, `scripts/tests/`, `docs/generated/`, `.github/workflows/`                                                                     | Policy diagrams are auto-generated via mmdr and CI-enforced.           |
| 7.1.1 | `crates/zamburak-core/src/i18n/mod.rs`, `crates/zamburak-core/src/i18n/localizer.rs`, `tests/integration/`                                | Core localization contracts enforce explicit localizer injection.      |
| 7.1.2 | `crates/zamburak-core/src/i18n/fluent_adapter.rs`, `crates/zamburak-core/src/i18n/localizations.rs`, `locales/`                           | Fluent adapters compose with host loaders and bundled resources.       |
| 7.2.1 | `crates/zamburak-policy/src/diagnostics.rs`, `crates/zamburak-agent/src/confirmation.rs`, `crates/zamburak-tools/src/`                    | User-facing diagnostics expose explicit localized rendering paths.     |
| 7.2.2 | `tests/integration/`, `tests/security/`, `docs/`                                                                                          | Localization fallback and integration behaviour are test-covered.      |
...
## 1. Design-contract and delivery baseline

### 1.1. Canonical policy and authority contracts

- [x] Task 1.1.1: Freeze canonical policy schema v1 in runtime loading paths.
...
- [x] Task 1.1.2: Implement explicit schema migration transforms and migration
      conformance tests.
  - Requirement signposts:
...
- [x] Task 1.1.3: Implement authority token lifecycle conformance checks.
  - Requirement signposts:
    - `docs/zamburak-design-document.md` section
...
### 1.2. Baseline tools, verification gates, and script standards

- [x] Task 1.2.1: Align toolchain and quality-gate baseline with repository
      configuration.
  - Requirement signposts:
...
- [x] Task 1.2.2: Wire phase-gate checks from verification targets into
...
- [x] Task 1.2.3: Establish automation script baseline for roadmap-delivered
      scripts.
  - Requirement signposts:
...
### 1.3. Pre-phase contract conformance gate

- [x] Task 1.3.1: Enforce design-level conformance suites before Phase 2 build
      work.
  - Requirement signposts:
    - `docs/zamburak-design-document.md` section
  ...
  - Dependencies: Tasks 1.1.1, 1.1.3, 1.2.2.
  - In scope: schema, sink enforcement, and authority lifecycle contract test
    gating.
  - Out of scope: Phase 2 feature implementation.
  - Completion criteria: phase-2 implementation is blocked until all required
    conformance suites pass.

### 1.4. `full-monty` repository mechanics and guardrails

- [x] Task 1.4.1: Add `full-monty` as a Git submodule and define fork rules.
  - Requirement signposts:
...
- [x] Task 1.4.2: Add `make monty-sync` with fork sync and verification gates.
  - Requirement signposts:
...
### 1.5. Track A (`full-monty`) upstream-friendly substrate
...
- [x] Task 1.5.3: Add generic snapshot extension bytes in `full-monty` when
      required.
  - Requirement signposts:
    - `docs/adr-001-monty-ifc-vm-hooks.md` section
...
- [x] Task 1.5.4: Enforce Track A compatibility and performance invariants.
  - Requirement signposts:
    - `docs/adr-001-monty-ifc-vm-hooks.md` section
...
### 1.6. Track B (Zamburak governance) integration workstream

- [x] Task 1.6.1: Add `crates/zamburak-monty` adapter crate for governed
      execution.
  - Requirement signposts:
  ...
  - Dependencies: Tasks 1.5.2 and 1.1.1.
...
- [x] Task 1.6.2: Add IFC core crate with `ValueId`-keyed dependency graph.
  - Requirement signposts:
...
- [x] Task 1.6.3: Wire `full-monty` observer events into IFC updates.
  - Requirement signposts:
    - `docs/adr-001-monty-ifc-vm-hooks.md` section
    ...
    - `docs/verification-targets.md` rows "IFC propagation" and
      "Control context".
  - Dependencies: Tasks 1.6.2 and 1.5.3.
...
- [x] Task 1.6.4: Gate external calls through policy decisions at runtime.
  - Requirement signposts:
    - `docs/adr-001-monty-ifc-vm-hooks.md` section
  ...
  - Dependencies: Tasks 1.6.3 and 1.1.2.
...
- [ ] Task 1.6.5: Add compatibility, security, and snapshot-governance suites.
  - Requirement signposts:
...
### 1.7. `full-monty` PR-ability process controls

- [ ] Task 1.7.1: Enforce fork patch-budget and naming constraints in review.
  - Requirement signposts:
...
- [ ] Task 1.7.2: Require upstream-shaped commits for `full-monty` deltas.
  - Requirement signposts:
...
- [ ] Task 1.7.3: Add continuous `git range-diff` drift checks for fork delta
      growth.
  - Requirement signposts:
    - `docs/adr-001-monty-ifc-vm-hooks.md` section
...
## 2. Core trust semantics and policy inputs

### 2.1. Information-flow model separation

- [ ] Task 2.1.1: Implement separate runtime representations for integrity,
  ...
  - Dependencies: Tasks 1.3.1 and 1.6.5.
...
- [ ] Task 2.1.2: Implement deterministic verification, endorsement, and
...
### 2.2. Strict-mode effect context propagation

- [ ] Task 2.2.1: Include execution-context summaries in every effect policy
      check.
  - Requirement signposts:
...
- [ ] Task 2.2.2: Add strict-mode side-channel regression tests for call
...
## 3. Mutable-state soundness and bounded provenance

### 3.1. Container mutation and opcode completeness

- [ ] Task 3.1.1: Implement versioned container-state propagation with alias
      safety.
  - Requirement signposts:
    - `docs/zamburak-design-document.md` section
...
- [ ] Task 3.1.2: Complete opcode and built-in information-flow coverage.
  - Requirement signposts:
...
### 3.2. Summary budgets and explainability

- [ ] Task 3.2.1: Implement dependency-summary fast path with bounded witness
      capture.
  - Requirement signposts:
...
- [ ] Task 3.2.2: Enforce fail-closed behaviour on budget overflow and redact
...
## 4. Tool and Model Context Protocol (MCP) trust boundaries

### 4.1. Tool catalogue and trust-class enforcement

- [ ] Task 4.1.1: Implement local pinned tool catalogue loading and hash
      validation.
  - Requirement signposts:
  ...
  - Completion criteria: runtime rejects tool binding when catalogue hash or
    version mismatches.
- [ ] Task 4.1.2: Enforce MCP trust classes and per-server capability budgets.
  - Requirement signposts:
...
### 4.2. Draft and commit side-effect governance

- [ ] Task 4.2.1: Implement draft-to-commit lineage checks for irreversible
      operations.
  - Requirement signposts:
    ...
    - `docs/repository-layout.md` sections `crates/zamburak-agent` and
      `crates/zamburak-tools`.
  - Dependencies: Tasks 2.2.1, 4.1.1.
...
## 5. Large language model (LLM) sink governance

### 5.1. Planner and quarantined sink policy enforcement

- [ ] Task 5.1.1: Implement sink-policy signatures for planner LLM (P-LLM) and
  ...
  - Requirement signposts:
    ...
    - `docs/repository-layout.md` sections `crates/zamburak-agent` and
      `crates/zamburak-tools`.
  - Dependencies: Tasks 2.1.1, 4.1.1.
...
- [ ] Task 5.1.2: Implement three-point sink enforcement architecture and
...
### 5.2. Privacy boundary and local-only compatibility

- [ ] Task 5.2.1: Add end-to-end privacy boundary integration tests.
  - Requirement signposts:
...
- [ ] Task 5.2.2: Implement local-only compatibility profile for provider
      abstraction.
  - Requirement signposts:
...
## 6. Audit hardening, evaluation, and automation delivery

### 6.1. Confidentiality-first audit pipeline

- [ ] Task 6.1.1: Implement summary-first audit record persistence.
  - Requirement signposts:
    ...
    - `docs/repository-layout.md` sections `crates/zamburak-policy` and
      `crates/zamburak-cli`.
  - Dependencies: Tasks 3.2.2, 5.1.2.
...
- [ ] Task 6.1.2: Implement tamper-evident audit chaining with retention
      controls.
  - Requirement signposts:
...
### 6.2. Verification progression and benchmark gates

- [ ] Task 6.2.1: Maintain mechanistic and security regression gates as
  ...
  - Requirement signposts:
    ...
    - `docs/zamburak-engineering-standards.md` section
      "Testing and verification evidence standards".
  - Dependencies: Tasks 2.2.2, 3.2.2, 4.1.2.
...
- [ ] Task 6.2.2: Integrate model-in-loop adversarial benchmark stage.
  - Requirement signposts:
    - `docs/zamburak-design-document.md` section
...
### 6.3. Scripted automation quality and CI integration

- [ ] Task 6.3.1: Implement benchmark and verification helper scripts using
  ...
  - Requirement signposts:
    ...
    - `docs/repository-layout.md` section "Root and operational files"
      (`scripts/`),
    - `docs/tech-baseline.md` section "Baseline usage contract".
  - Dependencies: Task 1.2.3 and Task 6.2.2.
...
- [ ] Task 6.3.2: Add script test suites and CI wiring for script artefacts.
  - Requirement signposts:
...
- [ ] Task 6.3.3: Implement automatic Mermaid diagram generation from
  ...
  - Requirement signposts:
    ...
    - `docs/repository-layout.md` section "Root and operational files"
      (`scripts/`).
  - Dependencies: Task 1.1.1, Task 6.3.1, and Task 6.3.2.
...
## 7. Localization and user-facing diagnostics

### 7.1. Injection-first localization foundation

- [ ] Task 7.1.1: Introduce core localization contracts and fallback-localizer
      behaviour.
  - Requirement signposts:
    - `docs/zamburak-design-document.md` sections
...
- [ ] Task 7.1.2: Implement optional Fluent adapters and embedded catalogue
      loading helpers.
  - Requirement signposts:
    - `docs/zamburak-design-document.md` sections
...
### 7.2. Localized diagnostic rollout and conformance

- [ ] Task 7.2.1: Refactor user-facing diagnostics to explicit localized
  ...
  - Dependencies: Tasks 5.1.2 and 7.1.1.
...
- [ ] Task 7.2.2: Add localization conformance tests and integration
      documentation.
  - Requirement signposts:
    - `docs/zamburak-design-document.md` sections
      "Fallback and Fluent layering semantics" and
  ...
  - Dependencies: Task 7.2.1 and Task 7.1.2.
